...
class TestSuiteDB(object):
    ...
    def __init__(self, v4db, name, test_suite, create_tables=False):
        ...
        CVRun.samples = relation(CVSample, back_populates='run',
                                 cascade="all, delete-orphan")
        ...
        for item in order_fields:
            if item.info_key in run_parameters:
                ...
                if item.info_key == 'run_order':
                    ...
                    raise ValueError("Supplied run is missing parameter: {}".
                                     format(item.info_key))
        ...
        for test_data in tests_data:
            ...
            for key, values in test_data.items():
                ...
                if field is None and key != 'profile':
                    raise ValueError("test {} metric {} does not map to a "
                                     "sample field in the reported suite".
                                     format(name, key))
